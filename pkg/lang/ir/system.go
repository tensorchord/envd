...
func (g *Graph) preparePythonBase(root llb.State) llb.State {
	...
	// envd-sshd
	logrus.Debugf(">> sshd image name: %s", types.EnvdSshdImage)
	sshd := root.File(llb.Copy(
		llb.Image(types.EnvdSshdImage), "/usr/bin/envd-sshd", "/var/envd/bin/envd-sshd",
		&llb.CopyInfo{CreateDestPath: true}), llb.WithCustomName(fmt.Sprintf("[internal] add envd-sshd from %s", types.EnvdSshdImage)))
